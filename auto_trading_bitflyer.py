    ...
    def trade_bitflyer_constoplimit(self, type, buysellprice, amount):
        ...
        if type == 'BUY' or type == 'buy':
            ...
            parameters =  [{ 'product_code' : product, 'condition_type' : 'STOP_LIMIT', 'side': 'BUY',
                            'price': str(buysellprice), 'size': str(amount), 'trigger_price': str(buysellprice)}]
    ...
    def cancle_order(self, id):
        ...
        while i>0:
            try:
                ...
                if order != {}:
                    remain_amount = float(order['size']) - float(order['executed_size'])
                    return(remain_amount)
                else:
                    return(0)
            except Exception:
                print('maybe executed before cancelling')
                time.sleep(5)
                order = self.get_orderbyid(id)
                if order != {}:
                    remain_amount = float(order['size']) - float(order['executed_size'])
...
if __name__ == '__main__':
    autoTrading = AutoTrading(holdflag=True, order_places={'exist':False,'type':'','id':0,'remain':0.0}, tradeamount=1000)
