...
class Tenlines:
    ...
    def get_curr_line_no(self, all_pricelines, price):
        for i in range(len(all_pricelines)):
            if all_pricelines[i] > price:
                return i - len(all_pricelines)/2;

        return len(all_pricelines)/2
    ...
    def create_tenlines(self,current_price, interval=1500):
        ...
        all_lines=[]
        lo_lines.reverse()
        all_lines.extend(lo_lines)
        all_lines.extend(hi_lines)
        print(all_lines)

        return all_lines


    def random_tenlines_run(self):
        print("random_tenlines_run starts")
        while True:
