    ...
    def simulate(self, num=100, periods="1m" ,end_offset=0):
        ...
        for t in range(50, len(all)):
            ...
            if not short and not long:
                ...
                elif all[t][2] >temp_line and temp_line >0:
                    # short over
                    short = False
                    short_over_price = temp_line
                    cash = (1 + (short_start_price - short_over_price) / short_start_price) * trading_cash
                    if cash < 0:
                        cash == 0
                    short_start_price = 0.
                ...
                elif all[t][3]<temp_line and temp_line>0:
                    long = False
                    long_over_price = temp_line
                    cash = (1 - (long_start_price - long_over_price) / long_start_price) * trading_cash
                    if cash < 0:
                        cash == 0
                    long_start_price = 0.
...
if __name__ == '__main__':
    ...
    for i in range(length):
        value,counter = hilo.simulate(num=7*24*1 + 50, periods="1H", end_offset=3600 * 24 * (i + 0))
        sum = sum + value
