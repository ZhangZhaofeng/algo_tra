...
class AutoTrading:
    ...
    def switch2middel(self, cur_price, checkins, position_suggest):
    ...
    def trade_hour_init(self, checkins, cur_price, hilo):
        ...
        if hilo[2] >= hilo[1] : # if close > hi
            ...
            if profit < -self.stop_loss_factor * cur_price:
                predict.print_and_write('Stop loss')
                position_suggest = self.switch2middel(cur_price, checkins, position_suggest)
            elif profit > self.minize_stop_profit and profit < self.last_profit * self.stop_profit_factor:
                predict.print_and_write('Stop profit')
                position_suggest = self.switch2middel(cur_price, checkins, position_suggest)
            elif checkins[1] < 0.0 and cur_price > (hilo[0] + hilo[1])/2:
                predict.print_and_write('Switch 2 middel')
                position_suggest = self.switch2middel(cur_price, checkins, position_suggest)
            elif checkins[1] > 0.0 and cur_price < (hilo[0] + hilo[1])/2:
                predict.print_and_write('Switch 2 middel')
                position_suggest = self.switch2middel(cur_price, checkins, position_suggest)
                ...
                predict.print_and_write('Nothing done')

        else:
            predict.print_and_write('Wati for a chance')

        #over stopprofit line, change to zero
    ...
    def trial_order(self, checkins, trial_loss_cut, starttime):
        ...
        while tdelta < 3600:
            ...
            if profit < pre_profit:
                ...
                if profit > 3000:
                    trial_loss_cut = max_profit * 0.333
