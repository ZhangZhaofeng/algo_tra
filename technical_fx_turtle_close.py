...
class HILO:
    ...
    def publish_current_hilo_price(self, num=100, periods="1H"):
        ...
        low_price_ma = self.get_short_price(close_price)
        high_price_ma = self.get_long_price(close_price)
        low_price_ma_half = self.get_short_price(close_price, 30)
        high_price_ma_half = self.get_long_price(close_price, 30)
